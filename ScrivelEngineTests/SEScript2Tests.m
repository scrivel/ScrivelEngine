...
- (void)testArguments
{
    NSString *str = @"[つむぎ animate in:3 moveBy:{0px, 100px} rotateBy:100 scaleTo:2 repeatBy:2 repeatIn:1 reverse:yes]";
    NSError *e = nil;
    ...
    XCTAssert(step.toKeyValues.count == 7, );
    XCTAssert([step.toKeyValues[@"in"] unsignedIntegerValue] == 3, );
    XCTAssert([step.toKeyValues[@"reverse"] boolValue] == YES, );
    SEPoint p = [step.toKeyValues[@"moveBy"] se_pointValue];
    XCTAssert(CGPointEqualToPoint(p, CGPointMake(0, 100)), );
}
